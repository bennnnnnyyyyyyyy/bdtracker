...
export function getSupabaseAdmin() {
  if (!SUPABASE_KEY) {
    throw new Error('Supabase service role or anon key not configured in environment.');
  }
...
export async function saveRawDataToSupabase(data: {
  ...
  if (data.meetings.length > 0) {
    // Clear & re-insert or upsert meetings
    await supabase.from('meetings').delete().neq('id', 0); // resets snapshot
...
/**
 * Reads raw records from Supabase PostgreSQL tables.
 */
export async function getRawDataFromSupabase(): Promise<{
  ...
  try {
    ...
    // 3. Fetch all calls (Supabase default limit is 1000, so we fetch in pages if > 1000)
